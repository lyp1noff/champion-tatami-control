...
import { Bracket, BracketMatch } from "@/lib/api";
...
export default function TatamiSetupPage({ params }: { params: Promise<{ id: string }> }) {
  const router = useRouter();

  const { id: tatamiId } = use(params);
  ...
  const fetchTournament = async () => {
    try {
      const response = await fetch(`http://10.26.1.90:8080/api/tournaments/${selectedTournament}`);
      const data = await response.json();
      setTournament(data);
    } catch (error) {
  ...
  const fetchMatches = async (bracketId: string) => {
    try {
      setLoading(true);
      const response = await fetch(`http://localhost:8080/api/brackets/${bracketId}/matches`);
      const data = await response.json();

      // Filter matches that have 2 participants
      const validMatches = data.filter(
        (bracketMatch: BracketMatch) => bracketMatch.match.athlete1 && bracketMatch.match.athlete2
      );
  ...
  const handleStartMatch = () => {
    ...
    // Store match configuration in localStorage
                  ...
                  <Select value={selectedMatch} onValueChange={handleMatchChange}>
                    ...
                    <SelectContent>
                      {matches.map((bracketMatch) => (
                        <SelectItem key={bracketMatch.match.id} value={bracketMatch.match.id}>
